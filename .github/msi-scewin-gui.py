...
class OptimizedNVRAMParser:
    ...
    def _parse_setting_block_optimized(self, block):
        ...
        try:
            ...
            # Enhanced binary option detection for 0/1 values
            if (setting.current_value in ('0', '1') and not setting.options):
                is_binary_setting = False
                
                # Check setup question for enable/disable keywords
                if ('enable' in setting.setup_question.lower() or 'disable' in setting.setup_question.lower()):
                    is_binary_setting = True
                
                # Check help string for enabled/disabled patterns
                if not is_binary_setting and setting.help_string:
                    help_lower = setting.help_string.lower()
                    if ('enabled' in help_lower and 'disabled' in help_lower):
                        is_binary_setting = True
                
                # Check comment lines in original block for binary patterns
                if not is_binary_setting and hasattr(setting, 'original_block_lines'):
                    for comment_line in setting.original_block_lines:
                        comment_line_lower = comment_line.lower().strip()
                        if comment_line_lower.startswith('//'):
                            if (('enabled' in comment_line_lower and '1' in comment_line_lower and 
                                 'disabled' in comment_line_lower and '0' in comment_line_lower) or
                                ('enable' in comment_line_lower and '1' in comment_line_lower and 
                                 'disable' in comment_line_lower and '0' in comment_line_lower)):
                                is_binary_setting = True
                                break
                
                if is_binary_setting:
    ...
    def _parse_setting_block(self, block):
        ...
        try:
            ...
            # Special handling: if numeric and value is 0 or 1, detect Enabled/Disabled patterns
            if value_line is not None and value_val is not None:
                try:
                    v = int(value_val)
                    if v in (0, 1):
                        # Check for enabled/disabled patterns in multiple places:
                        # 1. Help string
                        # 2. Setup question
                        # 3. Comment lines in original block (e.g., "// Enabled = 1, Disabled = 0")
                        is_binary_setting = False
                        
                        # Check help string and setup question
                        ...
                        # Check comment lines for binary patterns like "// Enabled = 1, Disabled = 0"
                        if not is_binary_setting:
                            for comment_line in block_lines:
                                comment_line_lower = comment_line.lower().strip()
                                if comment_line_lower.startswith('//'):
                                    # Look for patterns like "enabled = 1" and "disabled = 0"
                                    if (('enabled' in comment_line_lower and '1' in comment_line_lower and 
                                         'disabled' in comment_line_lower and '0' in comment_line_lower) or
                                        # Also check for "enable" and "disable" variants
                                        ('enable' in comment_line_lower and '1' in comment_line_lower and 
                                         'disable' in comment_line_lower and '0' in comment_line_lower)):
                                        is_binary_setting = True
                                        break
